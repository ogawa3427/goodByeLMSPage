...
(() => {
  ...
  const webExtAPI = (() => {
    if (typeof browser !== 'undefined' && browser.storage) {
      ...
      console.error('WebExtensions APIが利用できません');
      return null;
    }
  })();
  
  // データをstorage.localに保存する関数
  const saveToStorage = (courseData) => {
    try {
      ...
      if (webExtAPI && webExtAPI.storage) {
        webExtAPI.storage.local.set({ courseData: data }, () => {
          if (webExtAPI.runtime && webExtAPI.runtime.lastError) {
            console.error('ストレージへの保存に失敗:', webExtAPI.runtime.lastError);
            return;
          }
  ...
  const loadFromStorage = () => {
    return new Promise((resolve) => {
      ...
      webExtAPI.storage.local.get(['courseData'], (result) => {
        if (webExtAPI.runtime && webExtAPI.runtime.lastError) {
          console.error('ストレージからの読み込みに失敗:', webExtAPI.runtime.lastError);
          resolve(null);
          return;
        }
        ...
        if (now - data.timestamp > CACHE_EXPIRY) {
          ...
          webExtAPI.storage.local.remove('courseData');
          resolve(null);
          return;
        }
  ...
  // popupからのメッセージを受信
  if (webExtAPI && webExtAPI.runtime) {
    webExtAPI.runtime.onMessage.addListener((request, sender, sendResponse) => {
      ...
      if (request.action === 'fetchCourseData') {
        console.log('fetchCourseDataアクション実行');
        logStaffInfo();
  ...
  // 自動実行は無効化（popupからのメッセージでのみ実行）
  console.log('makeTable.js読み込み完了。popupからのメッセージ待機中...');
  
  // デバッグ用: ページ読み込み完了をログ出力
  if (document.readyState === 'complete') {
    console.log('document.readyState=complete: ページ読み込み完了');
  } else {
